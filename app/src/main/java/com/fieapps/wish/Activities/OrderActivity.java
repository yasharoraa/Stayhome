...
import com.fieapps.wish.Adapters.OrderItemAdapter;
import com.fieapps.wish.Interfaces.ImageUploadCallback;
import com.fieapps.wish.Interfaces.OrderItemAction;
...
public class OrderActivity extends AppCompatActivity implements View.OnClickListener, TextView.OnEditorActionListener, OrderItemAction {
            ...
            case R.id.button_continue:
                ...
                if (IS_IMAGE){
                    ...
                    if (adapter.getItemCount() < 2){
                        makeToast("Add at least two items to continue.");
                        return;
                    }
    ...
    private OrderItem getItem() {
        ...
        if (item.isEmpty() || item.length() < 3) {
            itemNameEditText.setError("Please enter a valid item");
...
//                return;
    ...
    public void onBackPressed() {
        if (LOADING_PROCESS_RUNNING) {
            showCancelDialog(OrderActivity.this, "Do you want to abort uploading process?");
            return;
        }
        super.onBackPressed();
    }
        ...
        switch (textView.getId()) {
            ...
            case R.id.quantity_edit_text:
                if (text.isEmpty() || !text.matches("\\d+(?:\\.\\d+)?") || Integer.parseInt(text) < 1) {
                    itemNameEditText.setError("Please enter a valid quantity");
                    itemNameEditText.requestFocus();
                    return true;
                }
                break;
